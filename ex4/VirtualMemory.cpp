...
void getFrameToEvict(uint64_t pageNum, uint64_t *frame, int *page) {
    ...
    for (int i = 0; i < NUM_PAGES; ++i) {
        ...
        int dist = static_cast<int>(fmin(abs((int) pageNum - i),
                                         NUM_PAGES - (abs((int) pageNum - i))));
...
int getFrame(int lastFound, uint64_t pageNum) {
    ...
    } else if (max_t < NUM_FRAMES - 1) //TODO how does max_T work?
    {
        return max_t + 1;
    } else {
        cout << "in else - suppose to call evict" << endl;
...
int VMwrite(uint64_t virtualAddress, word_t value) {
    ...
    PagesToFrames[pageNum][1] = father;
    return 1;
}
