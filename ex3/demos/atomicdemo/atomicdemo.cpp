...
int main(int argc, char **argv) {
    ...
    std::atomic<int> atomic_counter(
            0); // I think its a counter that uses atomic wy so we make sure that there are no context switch in the middle
    int bad_counter = 0;
    ...
    for (int i = 0; i < MT_LEVEL; ++i) {
        pthread_create(threads + i, NULL, foo, contexts +
                                               i); // create a new thread with the foo , and all the other parameters according to its num in the arrays
    }
